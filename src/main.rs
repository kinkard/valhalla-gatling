...
enum Commands {
    ...
    /// Throws away all requests that are outside of the specified bounding box.
    Cut {
        /// The playbook file in .playbook format, generated by the `extract` command.
        playbook: String,
        /// The bounding box to cut the requests by, in the format "left,bottom,right,top".
        bbox: String,
        /// Output file name
        #[arg(short, long)]
        output: String,
...
fn cut(playbook: String, bbox: String, output: String) {
    let playbook = Playbook::load(&playbook).expect("Failed to load playbook");
    println!(
        "Loaded {} requests from the playbook",
        playbook.requests.len()
    );

    let bbox: Vec<f64> = bbox
        .split(',')
        .map(|s| s.parse().expect("Invalid bounding box coordinate"))
        .collect();
    if bbox.len() != 4 {
        panic!("Bounding box must have 4 coordinates: min_lon,min_lat,max_lon,max_lat");
    }

    let filtered_requests: Vec<Request> = playbook
        .requests
        .into_iter()
        .filter(|r| {
            let Ok(api) = proto::api::Api::parse_from_bytes(&r.body) else {
                // If the request body is not a valid API request, we skip it
                return false;
            };

            // Well, Valhalla has quite a list of properties where locations can be found.
            std::iter::empty() // just to have all of them nicely formatted in the same way
                .chain(api.options.locations.iter())
                .chain(api.options.sources.iter())
                .chain(api.options.targets.iter())
                .chain(api.options.shape.iter())
                .chain(api.options.trace.iter())
                .map(|location| (location.ll.lat(), location.ll.lng()))
                .chain(PolylineIter::new(6, api.options.encoded_polyline()))
                .all(|(lat, lon)| {
    ...
    println!("Filtered requests: {}", filtered_requests.len());

    let new_playbook = Playbook {
        requests: filtered_requests,
    };
    new_playbook
        .save(&output)
        .expect("Failed to save cut playbook");
}
